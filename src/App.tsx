...
const registrarFiado = async () => {
  ...
  if (productoVariable) {
    ...
    const variableYaEstaEnCarrito = carritoFinal.some(
      (item) =>
    ...
    if (!variableYaEstaEnCarrito) {
      alert(
        "El producto variable no está en el carrito. Ingrese nuevamente el monto."
      );
      return;
    }
  }
  ...
  const totalCompra = carritoFinal.reduce(
    (total, item) => total + Number(item.total),
    0
  );
  
  const detalleProductos = carritoFinal
    .map(
      (item) =>
        `${item.icono} ${item.cantidad} x ${item.nombre}: $${Number(
          item.total
    ...
    const fiadosCliente = fiados.filter(
      (fiado) =>
        Number(fiado.cliente_id) === Number(clienteId)
    );
    
    const abonosCliente = abonos.filter(
      (abono) =>
        Number(abono.cliente_id) === Number(clienteId)
    );
    
    const saldoAnterior =
      fiadosCliente.reduce(
        (total, item) => total + Number(item.monto),
        0
      ) -
      abonosCliente.reduce(
        (total, item) => total + Number(item.monto),
        0
      );
    
    const nuevoSaldo =
      saldoAnterior + totalCompra;
      ...
      `🛒 *Minimarket Sara*\n\n` +
      `Cliente: ${clienteSeleccionado?.nombre || "Sin nombre"}\n\n` +
      `📦 *Detalle de la compra*\n` +
      `${detalleProductos}\n\n` +
      `💰 *Total compra: $${totalCompra.toLocaleString("es-CL")}*\n\n` +
      `📒 Saldo anterior: $${saldoAnterior.toLocaleString("es-CL")}\n` +
      `📌 *Total pendiente: $${nuevoSaldo.toLocaleString("es-CL")}*\n\n` +
  ...
  window.open(
    `${whatsappBase}?text=${encodeURIComponent(mensajeWhatsApp)}`,
    "_blank",
    "noopener,noreferrer"
  );
...
<input
  ...
  onChange={(event) => {
    ...
    setCarrito((carritoActual) => {
      ...
      const itemVariable: ItemCarrito = {
        productoId: null,
        nombre:
          descripcion || "Producto Variable",
        precioUnitario: Number(nuevoMonto || 0),
      ...
      if (indiceVariableActual >= 0) {
        return carritoActual.map(
          (item, index) =>
            index === indiceVariableActual
              ? itemVariable
              : item
        );
      }
